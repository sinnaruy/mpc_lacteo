...
if (isset($_SESSION['documento'])) {
    $documento = mysqli_real_escape_string($conexion, $_SESSION['documento']);
    $sql = "SELECT u.*, 
            COALESCE(u.nombre1, '') as nombre1,
            COALESCE(u.nombre2, '') as nombre2,
            COALESCE(u.apellido1, '') as apellido1,
            COALESCE(u.apellido2, '') as apellido2,
            COALESCE(u.direccion, '') as direccion,
            COALESCE(u.municipio, '') as municipio,
    ...
    if (!$res) {
        ...
        echo "No se encontró el usuario con documento: " . htmlspecialchars($documento);
    }
} else {
    echo "No hay documento en la sesión";
}
?>
...
<body>
  ...
  <nav class="navbar navbar-expand-lg bg-body-tertiary" style="background-color: rgb(196, 255, 227);">
    <div class="container-fluid">
      ...
      <a href="../vista/inicio.php"><button type="submit" class="btn btn-dark ms-4">Cerrar sesión</button></a>
    </div>
          ...
          <?php if ($usuario): ?>
            ...
            <ul class="list-group list-group-flush mb-3">
              <li class="list-group-item">
                ...
                <?php 
                    $nombreCompleto = trim($usuario['nombre1'] . ' ' . $usuario['nombre2'] . ' ' . 
                                        $usuario['apellido1'] . ' ' . $usuario['apellido2']);
                    echo htmlspecialchars($nombreCompleto); 
                ?>
              </li>
              <li class="list-group-item"><strong>Documento:</strong> <?php echo htmlspecialchars($usuario['documento']); ?></li>
              <li class="list-group-item"><strong>Correo:</strong> <?php echo htmlspecialchars($usuario['correo']); ?></li>
              <li class="list-group-item"><strong>Teléfono:</strong> <?php echo htmlspecialchars($usuario['telefono'] ?? 'No registrado'); ?></li>
              <li class="list-group-item"><strong>Dirección:</strong> <?php echo htmlspecialchars($usuario['direccion'] ?? 'No registrada'); ?></li>
              <li class="list-group-item"><strong>Ciudad:</strong> <?php echo htmlspecialchars($usuario['municipio'] ?? 'No registrada'); ?></li>
              <li class="list-group-item"><strong>Departamento:</strong> <?php echo htmlspecialchars($usuario['departamento'] ?? 'No registrado'); ?></li>
              <li class="list-group-item"><strong>Perfil:</strong> <?php echo htmlspecialchars($usuario['perfil']); ?></li>
            </ul>
          <?php else: ?>
            <div class="alert alert-danger text-center">
                No se pudo cargar la información del usuario. 
                <?php if (!isset($_SESSION['documento'])): ?>
                    <br>Por favor, inicie sesión nuevamente.
                <?php endif; ?>
            </div>
          <?php endif; ?>
        </div>
